 ...
 * Features: TFT Touch UI, Local Web Control, Cloud Control (MQTT), Physical Button
 * 
...
void updatePumpLogic() {
  ...
  switch (currentState) {
    ...
    case PumpState::DRY_RUN_LOCKED:
      ...
      // ONLY run countdown if Auto-Retry is greater than 0
  ...
  if (currentState != lastState || tankConfig.displayUpperPercentage != lastPercentage || voltageConfig.status != lastVoltStatus || pumpConfig.flowDetected != lastFlow || dryRunConfig.error != lastErr || pumpConfig.motorStatus != lastMotorStatus || tankConfig.errorAck != lastAck) {  
...
void updateLCD() {
  ...
  if (dryRunConfig.error == 1) info = " DRY ALRM ";
    ...
    if (dryRunConfig.autoRetryMinutes == 0) {
      ...
      int m = dryRunConfig.retryCountdown / 60;
      int s = dryRunConfig.retryCountdown % 60;
      char buf[11];
