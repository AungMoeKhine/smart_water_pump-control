...
struct TankConfig {
  ...
  void updateFullThreshold() {
    ...
    // Calculation: Set the 'Full' stop point at 80% of the tank's physical height
    // to prevent overflow and account for the sensor's dead zone.
...
<style>
  ...
  .badge { padding: 5px 12px; border-radius: 20px; font-size: 0.85rem; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; box-shadow: 0 2px 5px rgba(0,0,0,0.3); }
  .bg-success { background: rgba(40,167,69,0.2); color: #28a745; border: 1px solid rgba(40,167,69,0.5); }
  .bg-danger { background: rgba(220,53,69,0.2); color: #ff6b6b; border: 1px solid rgba(220,53,69,0.5); }
  .bg-warning { background: rgba(255,193,7,0.15); color: #ffc107; border: 1px solid rgba(255,193,7,0.5); }
  .bg-info { background: rgba(23,162,184,0.2); color: #17a2b8; border: 1px solid rgba(23,162,184,0.5); }
...
<script>
  ...
  const rfr = () => { if(navigator.vibrate) navigator.vibrate(20); let b=document.getElementById('rfb'); b.classList.add('spinning'); upd().finally(()=>b.classList.remove('spinning')); };
  ...
  const upd = () => { return fetch('/status').then(r=>r.json()).then(d=>{
    if(d.lang == 1) { document.body.classList.add('lang-mm'); applyDict(); } else document.body.classList.remove('lang-mm');
      ...
      if (d.tank <= 20) tf.classList.add('low-water'); else tf.classList.remove('low-water');
...
<style>
  ...
  <div class="card">
    ...
    <hr>
    <div style="text-align:center;"><h3 style="color:#d8b4fe; margin:0 0 15px 0;">🛠️ Maintenance</h3><button class="btn btn-purple" onclick="window.location.href='/update_github'">Check for Updates</button></div>
    <hr>
    <div style="text-align:center; background:rgba(40,167,69,0.05); border:1px solid rgba(40,167,69,0.3); border-radius:12px; padding:15px; margin-bottom:20px;">
      ...
      <div style="background:rgba(0,0,0,0.2); padding:10px; border-radius:8px; font-size:0.85rem; margin-bottom:15px; word-break:break-all;"><strong>Device ID:</strong> <span id="did" style="color:#03ef;">loading...</span></div>
  ...
  window.onload = () => {
    fetch('/config').then(r=>r.json()).then(c=>{
      ...
      let dD=""; for(let i=60; i<=180; i+=5) dD+=`<option value="${i}" ${c.dD==i?"selected":""}>${i} Seconds</option>`; document.getElementById('dD_s').innerHTML = dD;
       ...
       document.getElementById('did').innerText = c.did; toggleVDly();
       if(c.lang==1) {
...
void setup() {
  ...
  voltageConfig.RESUME_GAP = constrain(preferences.getInt("vGap", 5), 1, 10);
...
void networkTask(void* parameter) {
  ...
  while (true) {
    // Feed the Task Watchdog for Core 0 to prevent the ESP32 from rebooting
    esp_task_wdt_reset();  // Keep Core 0 alive
    ...
    server.handleClient();
    // Crucial: vTaskDelay allows the CPU to handle background WiFi/TCP tasks
    // and prevents the "Watchdog Triggered" crash.
...
void updatePumpLogic() {
  ...
  if (scheduleConfig.enabled) {
    struct tm timeinfo;
    if (getLocalTime(&timeinfo, 0)) {  // Removed the tm_year check to allow quicker sync
      int hour = timeinfo.tm_hour;
      if (scheduleConfig.dndStart > scheduleConfig.dndEnd) {
        // Case: 22:00 to 06:00
        if (hour >= scheduleConfig.dndStart || hour < scheduleConfig.dndEnd) {
          currentDndActive = true;
        }
      } else {
        // Case: 08:00 to 17:00
        if (hour >= scheduleConfig.dndStart && hour < scheduleConfig.dndEnd) {
